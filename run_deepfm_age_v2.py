...
from dnn.DeepFMSM import DeepFMSM
...
list_features = ['time', 'creative_id', 'ad_id', 'advertiser_id', 'product_id', ]
...
checkpoint = ModelCheckpoint('models/age_v2.h5', save_weights_only=False, save_best_only=True)
callbacks_list = [checkpoint,EarlyStopping(monitor='val_acc',patience=0)]


model = DeepFMSM(sparse_features, list_features,features_num_dict,k=10,list_k=100).model
model.compile("adam", "sparse_categorical_crossentropy",metrics=['sparse_categorical_crossentropy','acc'],)
...
model.save('models/age_v2_final.h5')